...
@Component({
  ...
  styleUrls: ['./cad-passageiro.component.css']
})
export class CadPassageiroComponent implements OnInit {
  ...
  public confirmarInscricao(formulario: NgForm): void {
    ...
    passageiro = new Passageiro(formulario.value.nomeCompleto,
    ...
    firebase.database().ref(`Passageiro/${btoa(passageiro.cpf)}`).set(passageiro).then(() => {
